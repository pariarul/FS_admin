...
import { Terms, TermsSection, Block } from '../types';

interface EditTermsProps {
  data: Terms;
  onSave: (updated: Terms) => void;
  onCancel: () => void;
}
...
/* --------------------------  COMPONENT  ------------------------ */
/* --------------------------------------------------------------- */
const EditTerms: React.FC<EditTermsProps> = ({ data, onSave, onCancel }) => {
  /* ---------- migrate legacy data (single description) ---------- */
  const migrate = (src: Terms): Terms => ({
    ...src,
  ...
  const [formData, setFormData] = useState<Terms>(migrate(data));
  ...
  const transformPayloadWithStrictOrder = (formData: Terms) => {
    return {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}/terms-conditions/update-terms-conditions`, {
        method: 'PUT',
  ...
  const translateToLang = async (
    ...
    const points: string[] = [];
    enBlocks.forEach(b => {
      if (b.type === 'description') descs.push(...b.text); // Flatten text array
