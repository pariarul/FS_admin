...
import AddProductsModal from "./components/AddProducts";
import EditProductModal from "./components/EditProduct";
import DeleteProductsModal from "./components/DeleteProducts";
...
interface ImportAPIResponse {
    import: Record<string, CategoryData>;
}
...
const ImportProductCMS = () => {
    const [importData, setImportData] = useState<ImportAPIResponse | null>(null);
    ...
    // FETCH IMPORT DATA (FIXED)
    useEffect(() => {
        const fetchImportData = async () => {
            try {
                const res = await fetch(`${baseURL}/products/get-import-products`);
                if (!res.ok) throw new Error("Failed to fetch import data");

                const json = await res.json();

                // API returns: { success: true, data: { import: { … } } }
                if (!json.success || !json.data?.import) {
                    throw new Error(json.message || "Invalid response format");
                }

                const apiResponse: ImportAPIResponse = { import: json.data.import };
                setImportData(apiResponse);

                // Default select first category's first product
                const categoryKeys = Object.keys(apiResponse.import);
                if (categoryKeys.length > 0) {
                    const firstCategoryKey = categoryKeys[0];
                    const firstCategory = apiResponse.import[firstCategoryKey];
        ...
        fetchImportData();
    }, [baseURL]); // include baseURL in deps if it's dynamic

    // FLATTEN PRODUCTS ACROSS CATEGORIES
    const allProducts: MultilingualProduct[] = [];
    if (importData) {
        Object.keys(importData.import).forEach((categoryKey) => {
            const cat = importData.import[categoryKey];
    ...
    const handleProductClick = (product: MultilingualProduct) => {
        const categoryData = importData?.import[product.categoryKey];
        if (!categoryData) return;

        setSelectedProduct({
            en: {
                ...product.en,
                category: categoryData.en.category, // Ensure the English category is set
            },
            zh: {
                ...product.zh,
                category: categoryData.zh.category, // Map the Chinese category
            },
            si: {
                ...product.si,
                category: categoryData.si.category, // Map the Sinhala category
            },
    ...
    const buildDeletePayload = (product: MultilingualProduct) => {
        const cat = importData?.import[product.categoryKey];
        return {
            ...
            <main className="flex-1 p-8 max-w-[1600px] mx-auto ml-64">
                ...
                <div className="flex gap-6">
                    ...
                    <div className="w-80 bg-white rounded-2xl shadow-lg border border-gray-100 h-[calc(100vh-180px)] overflow-hidden flex flex-col">
                        ...
                        <div className="overflow-y-auto flex-1 p-4">
                            {importData &&
                                Object.keys(importData.import).map((categoryKey) => {
                                    const cat = importData.import[categoryKey];
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-2 mb-4">
                                                    <h3 className="font-bold text-gray-700 text-lg">{lang === 'en' ? 'English' : lang === 'zh' ? 'Mandarin' : 'Sinhala'}</h3>
                                                </div>
                ...
                {productToDelete && (
                    <DeleteProductsModal
                        ...
                        onDelete={async (payload) => {
                            try {
                                const res = await fetch(`${baseURL}/products/delete-import-product`, {
                                    method: 'POST',
                ...
                {isModalOpen && <AddProductsModal
                    ...
                    importData={importData ? {
                        en: Object.values(importData.import).map(value => ({
                        ...
                        si: Object.values(importData.import).map(value => ({
                        ...
                        zh: Object.values(importData.import).map(value => ({
                        ...
                        categories: Object.fromEntries(
                            Object.entries(importData.import).map(([key, value]) => [
                                key,
                                {
                                    category: value.en.category,
                                    items: value.items.map(item => ({
                ...
                {isEditModalOpen && productToEdit && (
                    <EditProductModal
                        ...
                        product={{
                            category: importData?.import[productToEdit.categoryKey].en.category || '',
                            assetName: importData?.import[productToEdit.categoryKey].en.assetName || '',
                            imagePath: productToEdit.en.imagePath,
                            imageName: productToEdit.en.imageName,
                            origins: productToEdit.en.origins,
                            translations: {
                                zh: {
                                    category: importData?.import[productToEdit.categoryKey].zh.category || productToEdit.zh.category,
                                    imageName: productToEdit.zh.imageName,
                                    origins: productToEdit.zh.origins,
                                },
                                si: {
                                    category: importData?.import[productToEdit.categoryKey].si.category || productToEdit.si.category,
...
export default ImportProductCMS;