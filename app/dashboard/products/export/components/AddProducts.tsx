...
interface ExportData {
  ...
  si: ExportCategory[];
  zh: ExportCategory[];
  categories: { [key: string]: ExportCategory };
}
...
interface AddExportProductsModalProps {
  onClose: () => void;
...
/*                              Modal Component                               */
/* -------------------------------------------------------------------------- */
const AddExportProductsModal: React.FC<AddExportProductsModalProps> = ({
  onClose,
  exportData,
  selectedAssetName,
  ...
  /* ---------------------------------------------------------------------- */
  useEffect(() => {
    if (!selectedCategory || !exportData || !exportData.categories) return;

    const categoryKey = Object.keys(exportData.categories).find(
      (key) => exportData.categories[key]?.category === selectedCategory
    );
    if (!categoryKey || !exportData.categories[categoryKey]) {
      console.warn('No matching category key found for:', selectedCategory);
      return;
    }

    const categoryData = exportData.categories[categoryKey] as unknown as CategoryData;
...
- Origins: ${enOrigins}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const assetNameEntry = Object.entries(exportData?.categories ?? {}).find(
      ...
      const res = await fetch(`${baseURL}/products/add-export-product`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col">
        <div className="p-6 overflow-y-auto flex-grow">
          <h2 className="text-2xl font-bold mb-6 text-center text-primary">Add New Export Product</h2>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={(() => {
                            ...
                            const catData = exportData.categories[categoryKey];
...
export default AddExportProductsModal;